...
class MigrationManager:
    ...
    def migrate_issues(self):
        ...
        last_processed = self.redmine_issue_start_number
        if mapping_dict.keys():
            last_processed = int(max(mapping_dict, key=lambda x: int(x)))
        for json_fname in self.get_redmine_json_fnames():
            ...
            # Start processing at or after redmine_issue_START_number
            if not redmine_issue_num >= last_processed:
                msg('Skipping Redmine issue: %s (start at %s)' % (redmine_issue_num, last_processed ))
...
if __name__=='__main__':
    ...
    json_input_directory = os.path.join(REDMINE_ISSUES_DIRECTORY, project)
